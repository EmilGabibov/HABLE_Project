...
* **Nudge Rejection Handling:** A deterministic 4xx from `/api/social/nudge` (including the intentional participant 403) is marked processed in the local sync queue and surfaced through bounded diagnostics; transport/5xx failures remain retryable. The client must never weaken the Worker permission guard to avoid a retry.
