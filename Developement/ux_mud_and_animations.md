...
**Habit-card ring and emoji contract:** Custom-habit emoji is persisted as a leading title prefix and must be resolved through the shared habit-title parser anywhere the ring renders; no additional local database or sync field is needed. The standard card uses a `6px` idle ring and a `12px` established ring. Completion feedback may use the brief green success treatment, then settles with an ease-out transition to the configured habit color; when reduced motion is enabled, it settles directly. Home and dashboard use the same bounded `1200ms` completion-feedback window and the same preset-to-haptic mapping; reduced motion removes the transient delay while haptics still honor the user toggle. The footer progress bar is `6px`, and its day/streak labels use `12px` text. Partner avatar rings remain independently habit-colored.
