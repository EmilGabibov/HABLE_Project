...
- **Habit Form Contract:** In `HabitFormSheet`, verify create mode exposes the icon/title row, preset suggestions, intent copy, duration presets (`21`, `33`, `40`, `66`, `90`), color selection, and friend chips with avatar emoji. Confirm validation messages appear for an empty title or invalid duration, that edit mode swaps to `Save changes` and hides partner invites, and that selecting a preset updates the title/duration/help copy coherently.
