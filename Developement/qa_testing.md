...
- **Habit Card Responsive Layout:** At a narrow Android-sized viewport and a compact macOS window, verify Home and Dashboard habit cards keep the title/description, 120 px completion ring, feedback/skip affordance, day/streak footer, and 6 px progress bar inside the shared 264 px compact-card contract. Repeat with a long description and larger system text; no bottom/right overflow should be reported.
...
- **macOS Local Release Signing Boundary:** Run `flutter build macos --release` on a Mac without Apple signing identities and verify compilation succeeds without changing `macos/Runner/Release.entitlements`. Treat that artifact as compile-only; protected distribution archives must separately pass explicit signing overrides and the fail-closed distribution verifier.
...
- **Cross-Device Check-In Reconciliation:** Complete a habit on device A, sign the same user into device B, wait for authenticated daily sync, and verify the owner habit plus today's completed log are present in the local Drift-backed Home state. Repeat the sync and confirm no duplicate log is created; also verify a newer unsynced local mutation is not replaced by an older server snapshot.
