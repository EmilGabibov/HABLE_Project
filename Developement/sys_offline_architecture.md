...
* **Local Reminder State:** Daily reminder preferences live locally in Drift and only schedule OS notifications after an explicit user action. Android/iOS/macOS use the `flutter_local_notifications` adapter directly for supported OS permission and scheduling calls; macOS permission recovery opens System Settings when needed. Reminder enablement is a device-local concern; unsupported platforms fall back in-app. Supported PWA users may separately opt into Web Push from Profile; the remote subscription is delivery metadata only and notification taps return to the app for normal sync/Drift refresh.
